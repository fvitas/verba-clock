...
describe('serbian grid integrity', () => {
  ...
  it('hides the ВЕРБА easter egg in the fillers', () => {
    const cells = [serbian.rows[0][4], serbian.rows[1][8], serbian.rows[1][9], serbian.rows[1][10], serbian.rows[2][8]];
    expect(cells.join('')).toBe('ВЕРБА');
  });
});
